...
public class Demo {
	public static void main(String[] args) throws Exception {
		...
		IplImage grayImage = AbstractIplImage.create(width, height,
				IPL_DEPTH_8U, 1);
		...
		CanvasFrame frame = new CanvasFrame("Color Detector",
				CanvasFrame.getDefaultGamma() / grabber.getGamma());

		// Let's create some random 3D rotation...
		CvMat randomR = AbstractCvMat.create(3, 3), randomAxis = AbstractCvMat
				.create(3, 1);
		...
		while (frame.isVisible()
			...
			for (int i = 0; i < total; i++) {
				...
				cvRectangle(grabbedImage, cvPoint(x, y), cvPoint(x + w, y + h),
						AbstractCvScalar.RED, 1, CV_AA, 0);
				...
				cvFillConvexPoly(grabbedImage, hatPoints.position(0), 3,
						AbstractCvScalar.GREEN, CV_AA, 0);
			}
			...
			while (contour != null && !contour.isNull()) {
				if (contour.elem_size() > 0) {
					...
					cvDrawContours(grabbedImage, points,
							AbstractCvScalar.YELLOW, AbstractCvScalar.BLUE, -1,
							1, CV_AA);
				}
